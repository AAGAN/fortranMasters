...
!  Discussion:
!
!    An IMAT is a rectangular array of integer values.
!
!  Modified:
!
!    28 December 2004
...
subroutine s_to_i ( s, value, ierror, length )
...
!
  do node = 1, node_num
    read ( tec_file_unit, * ) node_coord(1:dim_num,node), node_data(1:node_data_num,node)
  end do
!
...
!
  ...
  do
    ...
    if ( ch_eqi ( name(1:1), 'N' ) .and. node_num == -1 ) then
      ...
      call s_to_i ( value, node_num, ierror, length )
      found_num = found_num + 1

    elseif ( ch_eqi ( name(1:1), 'E' ) .and. element_num == -1 ) then

      call s_to_i ( value, element_num, ierror, length )
      found_num = found_num + 1
